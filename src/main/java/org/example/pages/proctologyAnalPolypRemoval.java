...
public class proctologyAnalPolypRemoval {
    ...
    private By analPolypRemovalLink = By.xpath("//a[contains(text(), 'Anal Polyp Removal')]");

    public proctologyAnalPolypRemoval(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Proctology → Anal Polyp Removal
    public void hoverAndOpen() throws InterruptedException {
        ...
        allTreatments.clickSpecificTreatment("Anal Polyp Removal");

        System.out.println("✅ Opened Proctology → Anal Polyp Removal treatment page");
        Thread.sleep(1500);
    }
