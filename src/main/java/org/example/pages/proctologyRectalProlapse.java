...
public class proctologyRectalProlapse {
    ...
    private By rectalProlapseLink = By.xpath("//a[contains(text(), 'Rectal Prolapse')]");

    public proctologyRectalProlapse(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Proctology → Rectal Prolapse
    public void hoverAndOpen() throws InterruptedException {
        ...
        allTreatments.clickSpecificTreatment("Rectal Prolapse");

        System.out.println("✅ Opened Proctology → Rectal Prolapse treatment page");
        Thread.sleep(1500);
    }
