...
public class proctologyPerianalAbscess {
    ...
    private By perianalAbscessLink = By.xpath("//a[contains(text(), 'Perianal Abscess')]");

    public proctologyPerianalAbscess(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Proctology → Perianal Abscess
    public void hoverAndOpen() throws InterruptedException {
        ...
        allTreatments.clickSpecificTreatment("Perianal Abscess");

        System.out.println("✅ Opened Proctology → Perianal Abscess treatment page");
        Thread.sleep(1500);
    }
