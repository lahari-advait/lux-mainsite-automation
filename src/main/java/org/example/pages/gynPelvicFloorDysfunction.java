...
public class gynPelvicFloorDysfunction {
    ...
    private By gynSubmenu = By.xpath("//a[contains(text(), 'Gynaecology')]");
    private By pelvicFloorDysfunctionLink = By.xpath("//a[contains(text(), 'Pelvic Floor Dysfunction')]");

    public gynPelvicFloorDysfunction(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Gynaecology → Pelvic Floor Dysfunction
    public void hoverAndOpen() throws InterruptedException {
        parentWindow = driver.getWindowHandle();

        allTreatments.hoverOverTreatments();
        allTreatments.clickTreatmentCategory("Gynaecology");
        allTreatments.clickSpecificTreatment("Pelvic Floor Dysfunction");

        System.out.println("✅ Opened Gynaecology → Pelvic Floor Dysfunction treatment page");
        Thread.sleep(1500);
    }
