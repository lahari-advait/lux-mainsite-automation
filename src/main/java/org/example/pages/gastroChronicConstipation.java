...
public class gastroChronicConstipation {
    ...
    private By gastroSubmenu = By.xpath("//a[contains(text(), 'Gastroenterology')]");
    private By chronicConstipationLink = By.xpath("//a[contains(text(), 'Chronic Constipation')]");

    public gastroChronicConstipation(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Gastroenterology → Chronic Constipation
    public void hoverAndOpen() throws InterruptedException {
        parentWindow = driver.getWindowHandle();

        allTreatments.hoverOverTreatments();
        allTreatments.clickTreatmentCategory("Gastroenterology");
        allTreatments.clickSpecificTreatment("Chronic Constipation");

        System.out.println("✅ Opened Gastroenterology → Chronic Constipation treatment page");
        Thread.sleep(1500);
    }
