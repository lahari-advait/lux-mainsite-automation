...
public class proctologyFecalIncontinence {
    ...
    private By fecalIncontinenceLink = By.xpath("//a[contains(text(), 'Fecal Incontinence')]");

    public proctologyFecalIncontinence(WebDriver driver) {
        this.driver = driver;
        this.allTreatments = new allTreatments(driver);
    }

    // ✅ Full flow: Treatments → Proctology → Fecal Incontinence
    public void hoverAndOpen() throws InterruptedException {
        ...
        allTreatments.clickSpecificTreatment("Fecal Incontinence");

        System.out.println("✅ Opened Proctology → Fecal Incontinence treatment page");
        Thread.sleep(1500);
    }
